/**
 * Navbar — public navigation only. Admin dashboard is intentionally
 * excluded and accessible only via direct URL: /admin
 */
import { useState } from "react";
import { Link } from "wouter";
import { Button } from "@/components/ui/button";
import {
  Rocket, Wallet, ChevronDown, Menu, X, CheckCircle2,
  ArrowLeftRight, FileText, Repeat, BarChart3,
} from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="border-b border-pink-200/60 bg-white/90 backdrop-blur-md sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-6">
              ...
              <div className="hidden md:flex space-x-1">
                ...
                <Link href="/contact">
                  <Button variant="ghost" className="text-pink-700 hover:text-pink-500 hover:bg-pink-50 font-semibold">Contact</Button>
                </Link>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2 sm:space-x-3">
              ...
              <a href={TELEGRAM_URL} target="_blank" rel="noopener noreferrer"
                className="hidden sm:flex items-center justify-center w-9 h-9 rounded-full bg-pink-50 hover:bg-pink-100 border border-pink-200/60 transition-colors" title="Telegram">
              ...
              <a href={TWITTER_URL} target="_blank" rel="noopener noreferrer"
                className="hidden sm:flex items-center justify-center w-9 h-9 rounded-full bg-pink-50 hover:bg-pink-100 border border-pink-200/60 transition-colors" title="X (Twitter)">
              ...
              <a href={WHITEPAPER_URL} target="_blank" rel="noopener noreferrer"
                className="hidden lg:flex items-center justify-center w-9 h-9 rounded-full bg-pink-50 hover:bg-pink-100 border border-pink-200/60 transition-colors text-pink-600" title="Whitepaper">
                <FileText className="w-4 h-4" />
              </a>

              {/* Chain indicator */}
              {isConnected && currentChain && (
                <div className="hidden sm:flex items-center space-x-1.5 px-3 py-1.5 rounded-full border border-pink-200/60 bg-pink-50 text-sm font-semibold text-pink-700">
                  <ChainIcon chain={currentChain.icon} size={16} />
                  <span className="hidden lg:inline">{currentChain.name}</span>
                </div>
              )}
              {isConnected && !currentChain && (
                <div className="hidden sm:flex items-center px-3 py-1.5 rounded-full border border-amber-200 bg-amber-50 text-sm font-semibold text-amber-700">
              ...
              {/* Chain selector (when disconnected) */}
              {!anyConnected && (
              ...
              <Link href="/bridge" className="hidden sm:block">
                <button className="flex items-center justify-center w-9 h-9 rounded-full bg-pink-50 hover:bg-pink-100 border border-pink-200/60 transition-colors text-pink-600" title="Bridge">
                  <ArrowLeftRight className="w-4 h-4" />
                </button>
              </Link>
              <Link href="/swap" className="hidden sm:block">
                <Button variant="outline" className="border-pink-300/60 text-pink-700 hover:bg-pink-50 font-bold rounded-full px-4 shadow-sm">
                  <Repeat className="w-4 h-4 mr-1.5" /><span>Swap</span>
                </Button>
              </Link>
              <Link href="/launch" className="hidden sm:block">
                <Button className="bg-primary text-primary-foreground hover:bg-primary/90 font-bold rounded-full px-4 shadow-md">
              ...
              {anyConnected ? (
                <button onClick={() => setWalletOpen(true)}
                  className="wallet-btn px-4 py-2 rounded-full text-sm font-bold border-2 border-pink-300/60 shadow-md hover:shadow-pink-300/50 transition-all">
          ...
          {mobileOpen && (
            <div className="md:hidden border-t border-pink-100 py-3 space-y-1">
              {[
                { label: "Home", href: "/" },
                { label: "Launch Token", href: "/launch", Icon: Rocket },
                { label: "Portfolio", href: "/portfolio", Icon: BarChart3 },
                { label: "Swap", href: "/swap", Icon: Repeat },
                { label: "Bridge Assets", href: "/bridge", Icon: ArrowLeftRight },
                { label: "Verify Token", href: "/verify" },
              ...
              {anyConnected && (
                <button onClick={() => { disconnect(); solana.disconnect(); setMobileOpen(false); }}
                  className="mx-4 mt-1 flex items-center space-x-2 text-sm text-rose-500 font-semibold">
