...
const SWAPS = [
  {
    ...
    button: "bg-[#F3BA2F] hover:bg-[#e0a91f] text-pink-950",
  },
  {
    id: "base",
    name: "Uniswap · Base",
  ...
  },
  {
    ...
    button: "bg-pink-950 hover:bg-pink-900",
  },
  {
    id: "x1",
    name: "X1 Swap",
    ...
    <div className="max-w-4xl mx-auto py-8 pb-20 animate-in fade-in duration-500">
      <div className="mb-10 text-center">
        ...
        <h1 className="text-4xl font-extrabold tracking-tight mb-2">
          <span className="bg-gradient-to-r from-pink-500 via-pink-600 to-pink-700 bg-clip-text text-transparent">Swap Tokens</span>
        </h1>
          ...
          <a key={s.id} href={s.url} target="_blank" rel="noopener noreferrer" className="group">
            <div className={`relative h-full rounded-3xl border border-pink-100 bg-gradient-to-br ${s.accent} p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden`}>
              <div className="flex items-start justify-between mb-5">
                ...
                <ExternalLink className="w-4 h-4 text-pink-400 group-hover:text-pink-700 transition-colors" />
              </div>
              <h3 className="text-xl font-extrabold text-pink-900 mb-1.5">{s.name}</h3>
              <p className="text-sm text-pink-700 mb-4 leading-relaxed">{s.description}</p>
              <p className="text-xs font-mono text-pink-400 mb-6">{s.chains}</p>
